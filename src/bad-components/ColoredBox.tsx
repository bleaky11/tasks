...
function ChangeColor({setColorIndex, colorIndex}: colorIndexes): React.JSX.Element {
    const index = colorIndex
    return (
        <Button
            onClick={() => {
                setColorIndex((1 + index) % COLORS.length);
            }}
        >
            Next Color
        </Button>
...
function ColorPreview({colorIndex}: colorIndexes): React.JSX.Element {
    return (
        <div
